...
module.exports.request = {
  ...
  properties: {
    ...
    conversion_type: oneOf({
      type: 'string',
      description: 'The rules by which the destination amount should be calculated. "exchange_rate" will trigger a lookup in voucher_systems.exchange_rates, while "custom" will trigger the relevant hook from loyalty_systems.hooks',
      enum: [
        'exchange_rate',
        'custom'
      ]
    ...
    destination_type: oneOf({
      type: 'string',
      description: 'The entity where the values should be transferred to, e.g. "voucher"',
      enum: [
        'voucher'
      ]
    }),

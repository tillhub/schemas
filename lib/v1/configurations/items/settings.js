        ...
        counting_numbers: oneOf({
          ...
          properties: {
            preferred_initialization: oneOf({
              description: 'When local data ís missing - counting numbers can be initialized from the api table or - if available - from a secondary local storage (e.g. keychain on iOS)',
              type: 'string',
              enum: [
                'remote', // API data will be considered as primaray source of truth
                'local' // local data will be considered as primaray source of truth
              ],
              default: 'remote'
            })
          }
